...
const juce::String OscSenderManager::OSC_WET_DRY_PERCENT = "/HeartSync/WetDry";
...
void OscSenderManager::sendHeartRateData(float rawBpm, float smoothedBpm, float wetDryPercent)
{
    ...
    oscSender->send(juce::OSCAddressPattern(OSC_WET_DRY_PERCENT), wetDryPercent);
    ...
    bundle.addElement(juce::OSCMessage(juce::OSCAddressPattern(OSC_WET_DRY_PERCENT), wetDryPercent));
...
void OscSenderManager::sendIfEnabled(float raw, float smoothed, float wetDry)
{
    if (enabled)
    {
        sendHeartRateData(raw, smoothed, wetDry);
    }
}
