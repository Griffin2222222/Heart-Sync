        ...
        if (device.id == deviceId) {
...

            {
                ...
                if (onConnectedCallback)
                    onConnectedCallback(currentDevice);
            }
...
void BluetoothManager::setConnectionCallbacks(std::function<void(const BluetoothDevice&)> onConnected,
...
void BluetoothManager::processHeartRateData(const IBuffer& buffer)
{
    try {
        ...
        if (flags & 0x10)
        {
            ...
            {
                auto rrValue = reader.ReadUInt16();
                float rrMs = static_cast<float>(rrValue) / 1024.0f * 1000.0f;
        ...
        {
            measurementCallback(static_cast<float>(heartRate), rrIntervals);
        }
...
void BluetoothManager::startHeartRateSimulation()
{
    ...
    std::thread([this]() {
        ...
        while (connected.load()) {
            ...
            if (heartRate > 0.0f)
            {
                const float clamped = std::max(40.0f, std::min(180.0f, heartRate));
                const float rrMs = 60000.0f / clamped;
                rrIntervals.push_back(rrMs);
            }

            std::lock_guard<std::mutex> lock(callbackMutex);
            if (measurementCallback) {
                measurementCallback(heartRate, rrIntervals);
            }
